...
class LL1Parser
{
        ...
        { "S'", new Dictionary<string, List<string>>
            {
                ...
                { ")", new List<string> { "ε" } }, // epsilon
                { "$", new List<string> { "ε" } }   // epsilon at the end of input
            }
        },
    ...
    public static bool Parse(List<string> tokens)
    {
        ...
        while (stack.Count > 0)
        {
            ...
            {
                Console.WriteLine($"Matched terminal: {currentToken}");
                index++;
            }
            else if (parsingTable.ContainsKey(top)) // Non-terminal
            {
                if (parsingTable[top].ContainsKey(currentToken))
                {
                    ...
                    Console.WriteLine($"Applying rule: {top} → {string.Join(" ", production)}");
    ...
    public static void Main()
    {
        ...
        Console.WriteLine("Please enter an expression using the following symbols: '(', ')', 'e', 'm', 'x', 'y', 'd', 'b'.");
        Console.WriteLine("Note: Valid tokens are: e, m, x, y, d, b, (, ).");
        ...
        Console.WriteLine("\nParsing Result: " + (isValid ? "Valid" : "Invalid"));
    ...
    private static List<string> LexicalAnalysis(string input)
    {
        ...
        // Process each character in the input, excluding spaces
        foreach (char ch in input.Replace(" ", ""))
        {
            ...
            {
                Console.WriteLine($"Error: Invalid token found: '{ch}'.");
